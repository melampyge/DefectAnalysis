...
//
// mpicxx -Wl,-rpath=${HOME}/hdf5_parallel/lib -L${HOME}/hdf5_parallel/lib 
// -I${HOME}/hdf5_parallel/include -I${HOME}/Eigen 
// -lhdf5 find_defects_multi.cpp compute_defect.cpp misc_tools.cpp read_write.cpp -o fdef_multi
//
// mpirun -np 16 /usr/users/iff_th2/duman/Defects/Scripts/Defects/fdef_multi 
// /usr/users/iff_th2/duman/Defects/Output/outcpp.h5 /usr/users/iff_th2/duman/Defects/Output
//
// mpirun -np 8 valgrind --leak-check=full --track-origins=yes 
// /usr/users/iff_th2/duman/Defects/Scripts/Defects/fdef_multi 
// /usr/users/iff_th2/duman/Defects/Output/outcpp.h5 /usr/users/iff_th2/duman/Defects/Output
//
//
...
int main (int argc, char *argv[]) {
  ...
  int nsteps, nbeads, nsamp, npols;
  nsteps = nbeads = npols = 0;
  double lx, ly, dt, density, kappa, fp, bl, sigma;
  lx = ly = dt = density = kappa = fp = bl = sigma = 0.;
  read_sim_data(filename, nsteps, nbeads, npols, 
      lx, ly, dt, density, kappa, fp, bl, sigma);

  int nbpp[npols];
  read_integer_array(filename, "/sim/nbpp", nbpp);
  ...
  double rn = 0.35*rcut;	                    // inner radius of the shell where neighbor search is going to be performed
  double dn = 5.;		                          // depth of the shell (THIS IS NOT USED AT THE MOMENT!)
  int nr = 1;			                            // total number of recursions to search friends and then friends of friends and so on 
  ...
  */
  ...
  for (int step = 0; step < nsteps; step++) {
    ...
    gen_linked_list(llist, head, rcut, nsegx, nsegy, 
        nseg, nbeads, x, y, lx, ly);

    // calculate bead orientations and turn them headless 
    
    calc_headless_orient(phi, x, y, lx, ly, npols, nbpp[0]);
    
    // allocate the arrays for possible defect points 
    // at this timestep with some large buffer
    ...
    // and compute the defect strength per point. 
    // start the recursion loop for possible defect points.
    
    search_main_pts(xbuffer, ybuffer, dbuffer, check_again, 
        cnt_defects, xpoints, ypoints, npoints, 
        x, y, phi, head, llist, rcut, 
        nsegx, nsegy, rn, nn, nr, dcut, lx, ly, randi);
    ...
    // communicate number of defects across the processes 
    // to create the hyperslab dimensions for writing data
    ...
    write_h5_data(savefolder, step, xdefects, ydefects, ddefects, 
        total_num_of_defects, rank, size, count, offset, MPI_COMM_WORLD);
