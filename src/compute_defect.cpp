...
void average_order_param_matrix (double *qxx, double *qxy, double *qyy, int *counter, 
    ...
    double *head, double *llist, double rcut, int nsegx, int nsegy, 
    double lx, double ly) {
...
double compute_single_defect (double xd, double yd, double *x, double *y, double *phi, 
  ...
  average_order_param_matrix(qxx, qxy, qyy, counter, northants, 
      xd, yd, x, y, phi, head, llist, rcut, nsegx, nsegy, lx, ly);
...
void search_neighborhood (double xd, double yd, double dmax, double dlabel, double *xdefects, double *ydefects, 
    double *ddefects, int &cnt_defects, double *x, double *y, double *phi, double *head, double *llist, 
    double rcut, int nsegx, int nsegy, double rn, int nn, int nr, double dcut, 
  ...
  for (int i = 0; i < nn; i++) {
    ...
    // check for -1/2 defects and +1/2 defects with Metropolis Monte Carlo sampling, 
    // that is, only accept the point if it is better in terms of defect strength than the parent point
    ...
    if (point_is_defect) {
      ...
      search_neighborhood(xn, yn, dst, dlabel, xdefects, ydefects, ddefects, cnt_defects, 
          x, y, phi, head, llist, rcut, nsegx, nsegy, rn, nn, nr, dcut, lx, ly, randi, cnt_recursion);
    }   
  }
...
void search_main_pts (double *xdefects, double *ydefects, double *ddefects, int *check_again, 
    int &cnt_defects, double *xpoints, double *ypoints, int npoints, double *x, double *y, 
    double *phi, double *head, double *llist, double rcut, int nsegx, int nsegy, 
    double rn, int nn, int nr, double dcut, double lx, double ly, MTRand randi) {
  ...
  for (int i = 0; i < npoints; i++) {
    ...
    if (point_is_defect) {
      ...
      search_neighborhood(xpoints[i], ypoints[i], dst, dlabel, xdefects, ydefects, ddefects, cnt_defects, 
          x, y, phi, head, llist, rcut, nsegx, nsegy, rn, nn, nr, dcut, lx, ly, randi, cnt_recursion);
    }
